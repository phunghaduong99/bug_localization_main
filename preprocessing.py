...
class ReportPreprocessing:
    ...
    def pos_tagging(self):
        ...
        for report in self.bug_reports.values():
            ...
            # Tokenizing using word_tokenize for more accurate pos-tagging
            summ_tok = nltk.word_tokeswtnnize(report.summary)
    ...
    def split_camelcase(self):
        ...
        for report in self.bug_reports.values():
            ...
            report.pos_tagged_summary = self._split_camelcase(
                report.pos_tagged_summary)
            report.pos_tagged_description = self._split_camelcase(
...
class SrcPreprocessing:
    ...
    def tokenize(self):
        ...
        for src in self.src_files.values():
            ...
            for idx, method in enumerate(src.methods):
                src.methods[idx] = nltk.wordpunct_tokenize(method)

            for idx, api_method in enumerate(src.methods_api):
                if api_method:
                    src.methods_api[idx] = nltk.wordpunct_tokenize(api_method)
    ...
    def split_camelcase(self):
        ...
        for src in self.src_files.values():
            ...
            src.pos_tagged_comments = self._split_camelcase(
                src.pos_tagged_comments)
            for idx, method in enumerate(src.methods):
                src.methods[idx] = self._split_camelcase(method)
            for idx, api_method in enumerate(src.methods_api):
                if api_method:
                    src.methods_api[idx] = self._split_camelcase(api_method)
    ...
    def normalize(self):
        ...
        for src in self.src_files.values():
            ...
            methods_punctnum_rem = []
            for idx, method in enumerate(src.methods):
                methods_punctnum_rem.append([token.translate(punctnum_table)
                                             for token in method])

            methods_api_punctnum_rem = []
            for idx, api_method in enumerate(src.methods_api):
                if api_method:
                    methods_api_punctnum_rem.append([token.translate(punctnum_table)
                                                     for token in api_method])
                else:
                    methods_api_punctnum_rem.append([])
            ...
            for idx, method in enumerate(src.methods):
                src.methods[idx] = [token.lower() for token
                                    in methods_punctnum_rem[idx] if token]

            for idx, api_method in enumerate(src.methods_api):
                src.methods_api[idx] = [token.lower() for token
                                        in methods_api_punctnum_rem[idx] if token]
            # src.methods = [token.lower() for token
            #                            in methods_punctnum_rem if token]
    ...
    def remove_stopwords(self):
        ...
        for src in self.src_files.values():
            ...
            src.pos_tagged_comments = [token for token in src.pos_tagged_comments
                                       if token not in stop_words]
            for idx, method in enumerate(src.methods):
                src.methods[idx] = [token for token in method
                                    if token not in stop_words]
            for idx, api_method in enumerate(src.methods_api):
                src.methods_api[idx] = [token for token in api_method
                                        if token not in stop_words]
    ...
    def stem(self):
        ...
        for src in self.src_files.values():
            ...
            for idx, method in enumerate(src.methods):
                src.methods[idx] = dict(zip(['stemmed', 'unstemmed'],
                                            [[stemmer.stem(token) for token in method],
                                             method]))

            for idx, api_method in enumerate(src.methods_api):
                src.methods_api[idx] = dict(zip(['stemmed', 'unstemmed'],
                                                [[stemmer.stem(token) for token in api_method],
                                                 api_method]))
...
def main():
    ...
    src_prep.preprocess()
    with open('data_output/preprocessed_src.pickle', 'wb') as file:
        pickle.dump(src_prep.src_files, file, protocol=pickle.HIGHEST_PROTOCOL)

    report_prep = ReportPreprocessing(parser.report_parser_txt())
    report_prep.preprocess()
    with open('data_output/preprocessed_reports.pickle', 'wb') as file:

...
public abstract class AccordionControl extends Composite
{
	...
	/**
	 * Creates the container which will hold the items in a category; this can
	 * be overridden to lay out the children with a different layout than the
	 * default vertical RowLayout
	 */
	 ...
	 * 
	 * @param parent
	 *            the parent composite to add the SWT items to
	 * @param header
	 *            the header object that is being opened for the first time
	 */
	 ...
	 * 
	 * @param single
	 *            if true, enforce a single category open at a time
	 */
	...
	/**
	 * Returns whether a single category should be enforced or not
	 * (default=true)
	 * 
	 ...
	 * 
	 * @param performLayout
	 *            if true, call {@link #layout} and {@link #pack} when done
	 */
	 ...
	 * 
	 * @param parent
	 *            the parent widget to add the accordion to
	 * @param style
	 *            the SWT style mask to use
	 * @param headers
	 *            a list of headers, whose {@link Object#toString} method should
	 *            produce the heading label
	 * @param greedy
	 *            if true, grow vertically as much as possible
	 * @param wrapChildren
	 *            if true, configure the child area to be horizontally laid out
	 *            with wrapping
	 * @param expand
	 *            Set of headers to expand initially
	 */
			...
			boolean greedy, boolean wrapChildren )
	{
		...
		for ( int i = 0; i < headers.size( ); i++ )
		{
			...
			final AccordionLabel label = new AccordionLabel( this,
			...
			if ( labelFont == null )
			{
				...
				FontData bold = new FontData( normal.getName( ),
						normal.getHeight( ),
						SWT.BOLD );
			...
			GridData scrollGridData = new GridData( SWT.FILL, greedy ? SWT.FILL
					: SWT.TOP, false, greedy, 1, 1 );
			...
			if ( wrapChildren )
			{
				scrolledComposite.addControlListener( new ControlAdapter( ) {
					...
					public void controlResized( ControlEvent e )
					{
						...
						{
							Point minSize = content.computeSize( r.width,
									SWT.DEFAULT );
	...
	private void setContentArea( final AccordionLabel label,
	...
	/**
	 * Returns the set of expanded categories in the palette. Note: Header
	 * labels will have escaped ampersand characters with double ampersands.
	 * 

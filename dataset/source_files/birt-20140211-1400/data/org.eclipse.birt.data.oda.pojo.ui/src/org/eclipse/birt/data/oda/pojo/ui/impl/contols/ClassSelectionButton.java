...
public class ClassSelectionButton
{
	...
	public void handleSelection( String[] paths, String rootPath,
			boolean isRelative )
	{
	...
	private ClassPathElement[] createClassPathElements( String[] paths,
			String rootPath, boolean isRelative )
	{
		ClassPathElement[] elements = new ClassPathElement[paths.length];

		for ( int i = 0; i < paths.length; i++ )
		{
			String fullPath = rootPath == null ? paths[i] : rootPath
					+ File.separator + paths[i];
			elements[i] = new ClassPathElement( new File( paths[i] ).getName( ),
					fullPath,
					isRelative );
		}

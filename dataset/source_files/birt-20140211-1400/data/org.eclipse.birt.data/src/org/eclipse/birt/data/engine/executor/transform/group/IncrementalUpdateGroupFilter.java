 ...
 * info.
 * <p>
 * While filtering groups, the group info and aggregation values are incrementally updated.
 * 
 * @author fengbin
 * 
 */
public class IncrementalUpdateGroupFilter extends IncrementalUpdateCaculator
{
	...
	private void populateFilterExpression( ) throws DataException
	{
		...
		if ( exprsList.size( ) > 0 )
		{
			...
			populator.getGroupProcessorManager( )
					.setExpressionProcessor( populator.getExpressionProcessor( ) );
			populator.getGroupProcessorManager( )
					.calculateExpressionList( exprsList,
							levelList,
	...
	private void seekToGroup( int groupLevel, int groupIdx )
	...
	private void doSinglePassFilter( int level, List<IFilterDefinition> filters )
			throws DataException
	{
		doFilterPass( level, filters, groupSize[level - 1] );
	}
	
	private void doMultiPassFilter( int level, List<IFilterDefinition> filters )
			throws DataException
	{
		if ( level > 1 )
		{
	...
	private void doEvaluatePass( int level, List<IFilterDefinition> filters,
	...
	private void doFilterPass( int level, List<IFilterDefinition> filters,
			int count ) throws DataException
	{
		for ( int i = 0; i < count; i++ )
		{
	...
	private void setFilteringResults( ) throws DataException
	{
		...
		populator.getGroupProcessorManager( )
				...
				.setResultSetCache( resultSetCache );
		populator.getGroupProcessorManager( )
				.getGroupCalculationUtil( )
		...
		for ( int i = 0; i < aggrs.length; i++ )
		{
			...
			aggrValuesUpdators[i] = new AggrValuesUpdator( aggrs[i],
					populator );
		}
		...
		{
			groupUpdators[i] = new GroupInfoUpdator( i,
					tempDir,
					originGroups[i],
					getLastGroupIndex( i ),
					aggrValuesUpdators );
	...
	private void acceptGroup( int level, int groupIndex ) throws DataException
	{
		...
		for ( int i = childChunck.first; i < childChunck.length
				+ childChunck.first; i++ )
		{
			assert i == populator.getResultIterator( ).getCurrentResultIndex( );
			IResultObject res = populator.getResultIterator( )
					.getCurrentResult( );
	...
	private boolean evaluateFilters( List<IFilterDefinition> filters )
			throws DataException
	{
		for ( int j = 0; j < filters.size( ); j++ )
		{
	...
	private Object evaluteFilterExpression( IFilterDefinition filter )
			throws DataException
	{
		IBaseExpression expr = filter.getExpression( );
		FilterUtil.prepareFilterExpression( tempDir,
				expr,
				filterPassCtrl,
				this.populator.getEventHandler( ).getExecutorHelper( ) );

		Object result = ScriptEvalUtil.evalExpr( expr,
				scriptCtx.newContext( this.populator.getGroupProcessorManager( )
